...
$sql = 'SELECT `studname`, `topic`,`id`,`roll` FROM `stud2`where `status`=0 and `branch`="EE"';
    ...
    if($result->num_rows == 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "
                    ...
                    <a href='ee.php?id={$row['roll']}'>{$row['roll']}</a>
                    {$row['topic']}<br/>";
...
// Query for a list of all existing files
$sql = 'SELECT `studname`, `topic`,`id`,`roll`,`text` FROM `stud2`where `status`=6 and `branch`="EE"';
$result = $dbLink->query($sql);
