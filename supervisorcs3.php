...
$sql = 'SELECT `studname`, `topic`,`id`,`roll` FROM `stud2`where `status`=0';
...
// Query for a list of all existing files
$sql = 'SELECT `studname`, `topic`,`id`,`roll`,`text` FROM `stud2`where `status`=6 and `branch`="CSE"';
$result = $dbLink->query($sql);
...
if($result) {
    ...
    if($result->num_rows == 0) {
        ...
        while($row = $result->fetch_assoc()) {
                 ...
                 echo    "<a href='cs.php?id={$row['roll']}'>{$row['roll']};
                  {$row['topic']}</a><br/>";
...
?>
	...
	<center>
	<div id="box1">
    <H3>THESIS REJECTED BY THE DEAN</H3> 
	<?php
// Connect to the database
$dbLink = new mysqli('127.0.0.1', 'root', '', 'sanjit');
if(mysqli_connect_errno()) {
...
// Query for a list of all existing files
$sql = 'SELECT `studname`, `topic`,`id`,`roll`,`text` FROM `stud2`where `status`=9 and `branch`="CSE"';
$result = $dbLink->query($sql);
